...
const WeaponInfo = (props) => {
    ...
    useEffect(() => {
        ...
        if(storage.find((item) => item.name === props.wp.name)) {
    ...
    const handleAddFocusButton = () => {
        ...
        localStorage.setItem('your_focus', JSON.stringify([...storage, {
            name: props.wp.name,
            image: props.wp.images.nameicon,
            url: props.wp.name.replace(/(\s)|(')/g, '').toLowerCase(),
            category: "weapon",
            weapontype: true
        }]))
        setIsLoadingFocus('ADDED')
    ...
    const handleRemoveFocusButton = () => {
        setIsLoadingFocus('PROCESS')
        const storage = JSON.parse(localStorage.getItem("your_focus")).filter((item) => item.name !== props.wp.name)
        localStorage.setItem('your_focus', JSON.stringify(storage))
        setIsLoadingFocus("ADD")
    }
                            ...
                            <table className="mt-3">
                                <tbody>
                                    <tr>
                                        ...
                                        <td className='px-3 container-box__text bold align-top'><Link data-testid={"link-material"} to={"/mats/" + props.wp.weaponmaterialtype.replace(/\s/g, '').toLowerCase()} className="font-bold underline">{props.wp.weaponmaterialtype}</Link></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
